...
def open_cameras():
    ...
    # Initialize separate command objects for each camera
    try:
        # Create command interfaces for each camera by index
        cam_cmd0 = P2Pro_CMD.P2Pro(device_idx=0)  # First thermal camera
        cam_cmd1 = P2Pro_CMD.P2Pro(device_idx=1)  # Second thermal camera
        
        # Set different color palettes to verify independent control
        cam_cmd0.pseudo_color_set(0, P2Pro_CMD.PseudoColorTypes.PSEUDO_IRON_RED)
        cam_cmd1.pseudo_color_set(0, P2Pro_CMD.PseudoColorTypes.PSEUDO_RAINBOW_1)
        
        print("Camera 0 palette:", cam_cmd0.pseudo_color_get())
        print("Camera 1 palette:", cam_cmd1.pseudo_color_get())
        
    except Exception as e:
        print(f"Error initializing camera commands: {e}")
        return
